...
const unordered_set<string> HttpRequest::DEFAULT_HTML(Helper::Instance()->DEFAULT_HTML);

const unordered_map<string, int> HttpRequest::DEFAULT_HTML_TAG = {
    {"/register.html", 0}, {"/login.html", 1},
};

void HttpRequest::FindFiles_(string root, vector<string>& files) {
    DIR* dir;
    struct dirent* ptr;
...
void HttpRequest::ParsePath_() {
    ...
    if (path_ == "/") path_ = "/index.html";
        ...
        for (auto &item: DEFAULT_HTML) {
            // if (item == path_) {
            if (strstr(item.c_str(), path_.c_str())) {
                if (path_.back() == '/')
                    path_ += "index.html";
                else path_ += "/index.html";
                break;
            }
        }
    }
